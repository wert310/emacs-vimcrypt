;;; blowfish.el --- encrypt and decrypt using Schneier's Blowfish cipher
;;
...
;;; Commentary:
;;
;;  This library provides a pure emacs-lisp implementation of the
;;  blowfish cipher.
;;
;;  The code is loosely based on https://en.wikipedia.org/wiki/Blowfish_(cipher)
;;
;;; Code:
...
(defun blowfish--32bit-blocks-of (str)
  (assert (= 0 (mod (length str) 4)))
  (do ((i 0 (+ i 4)) (res nil)) ((>= i (length str)) (nreverse res))
    (push (blowfish--unpack-u32 (substring str i (+ i 4))) res)))

(defun blowfish--unpack-u32 (str)
  (assert (= 4 (length str)))
  (cdar (bindat-unpack '((:v u32)) str)))

(defun blowfish--pack-u32 (n)
...
(defun blowfish-decode-hex (str)
  (loop with res = nil
        ...
        for byte = (string-to-number (substring str (* 2 i) (* 2 (1+ i))) 16)
        collecting (byte-to-string byte) into res
        finally (return (apply #'concat res))))
...
(defun blowfish--encrypt (L R P S)
  (do ((i 0 (+ i 2))) ((>= i 16))
    (setf L (logxor L (aref P i)))
    (setf R (logxor R (blowfish--f L P S)))
    (setf R (logxor R (aref P (1+ i))))
    (setf L (logxor L (blowfish--f R P S))))
  (setf L (logxor L (aref P 16)))
  (setf R (logxor R (aref P 17)))
  (list R L))

(defun blowfish--decrypt (L R P S)
  (do ((i 16 (- i 2))) ((<= i 0))
    (setf L (logxor L (aref P (1+ i))))
    (setf R (logxor R (blowfish--f L P S)))
    (setf R (logxor R (aref P i)))
    (setf L (logxor L (blowfish--f R P S))))
  (setf L (logxor L (aref P 1)))
  (setf R (logxor R (aref P 0)))
  (list R L))
...
(defun blowfish-init (key)
  ...
  (let ((key-array (apply #'vector (blowfish--32bit-blocks-of key))))
    (let ((P (apply #'vector (coerce Parray 'list)))
          (S (make-vector 4 nil)))
      (dotimes (i 4)
        (setf (aref S i) (apply #'vector (coerce (aref Sbox i) 'list))))
      (dotimes (i 18)
        (setf (aref P i) (logxor (aref Parray i)
                                 (aref key-array (mod i (length key-array))))))
      (let ((L 0) (R 0))
        (do ((i 0 (+ i 2))) ((>= i 18))
          (destructuring-bind (nl nr) (blowfish--encrypt L R P S)
            (setf L nl R nr))
          (setf (aref P i) L)
          (setf (aref P (1+ i)) R))
        (dotimes (i 4)
          (do ((j 0 (+ j 2))) ((>= j 256))
            (destructuring-bind (nl nr) (blowfish--encrypt L R P S)
              (setf L nl R nr))
            (setf (aref (aref S i) j) L)
            (setf (aref (aref S i) (1+ j)) R))))
...
(defun blowfish-decrypt (bf block)
  ...
  (destructuring-bind (L R) (blowfish--32bit-blocks-of block)
    (apply #'concat
           (mapcar #'blowfish--pack-u32
                   (blowfish--decrypt L R (blowfish-P bf) (blowfish-S bf))))))


;; TEST
(defun test-blowfish ()
  (let ((bb (blowfish-init (blowfish-decode-hex (secure-hash 'sha256 "ciao")))))
